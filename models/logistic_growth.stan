functions{
  real log_growth(real A, real mu, real L, real B, real t){
    return A*L*exp(t/mu)/(A+L*(exp(t/mu)-1)) + B;
  }
}
...
model {
  ...
  for (k in 1:K) {
    for (i in pos:(pos+idx[k]-1)) {
        OD[i] ~ normal(log_growth(A_rep[k], mu_rep[k], L_rep[k], B_rep[k], time[i]), sigma_OD);
    }
        pos <- pos + idx[k];
  }
}
...
for (i in 1:101){
    ...
    for (k in 1:K){
      OD_pred[i,k] <- log_growth(A_rep[k], mu_rep[k], L_rep[k], B_rep[k], time_pred[i]);
    }
    OD_pred_all[i] <- log_growth(A, mu, L, B, time_pred[i]);
}
}